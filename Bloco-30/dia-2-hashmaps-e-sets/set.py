...
#  A diferença de um superconjunto e de um subconjunto é que no superconjunto podem haver outros elementos, além dos
#   elementos de other já presentes dentro de set. Já no subconjunto não;

# set == other: verifica se os conjuntos são iguais, ou seja, se todos os elementos de set estão em other e se todos 
# os elementos de other estão em set. Lembre-se que a ordem não importa. Não existe função dedicada para a comparação 
# de igualdade.
...
# set.symmetric_difference(others): retorna todos os elementos que estejam em um mas
#  não estejam no outro conjunto (opera apenas sobre dois conjuntos).